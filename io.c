...
#ifdef ST_SSL
int st_netfd_ssl_init(_st_netfd_t *fd)
{
  SSL *ssl = NULL;
...
int st_netfd_close_keep_osfd(_st_netfd_t *fd)
{
#ifdef ST_SSL
  if (NULL != fd) {
    if (1 == fd->withSSL) {
...
#ifdef ST_SSL
int st_connect_ssl(_st_netfd_t *fd, const struct sockaddr *addr, int addrlen,
	       st_utime_t timeout)
{
  int n, err = 0;
  int ret_ssl = 0;
  ...
  if (1 == fd->withSSL) {
    SSL_set_fd(fd->ssl, fd->osfd);
    SSL_set_connect_state(fd->ssl);

    while ((ret_ssl = SSL_do_handshake(fd->ssl)) != 1) {
      if (errno == EINTR)
        continue;
      if (!_IO_NOT_READY_ERROR)
        return -1;

      err_ssl = SSL_get_error(fd->ssl, ret_ssl);
      if (SSL_ERROR_WANT_READ == err_ssl) {
        events &= ~POLLOUT;
        events &= POLLIN;
      } else if (SSL_ERROR_WANT_WRITE == err_ssl) {
        events &= ~POLLIN;
        events &= POLLOUT;
      }

      if (st_netfd_poll(fd, events, timeout) < 0)
        return -1;
...
#ifdef ST_SSL
  if (1 == fd->withSSL) {
    while ((n = SSL_read(fd->ssl, buf, nbyte)) < 0) {
      if (errno == EINTR)
        continue;
      if (!_IO_NOT_READY_ERROR)
        return -1;
		    ...
		    st_utime_t timeout)
{
  ...
  while (*iov_size > 0) {
    ...
    {
      if (1 == fd->withSSL) {
        n = SSL_write(fd->ssl, (*iov)->iov_base, (*iov)->iov_len);
      } else {
